...
void setup() {
  ...
  //   build_path: esphome_pow_sw1
  //   platformio_options: {}
  //   esp8266_restore_from_flash: false
  //   includes: []
  //   libraries: []
  App.pre_setup("esphome_pow_sw1", __DATE__ ", " __TIME__);
  // binary_sensor:
  // switch:
  // light:
  // logger:
  ...
  wifi_wifiap.set_manual_ip(wifi::ManualIP{
      .static_ip = IPAddress(192, 168, 0, 36),
  ...
  light_lightstate = new light::LightState("esphome pow sw1 LED", monochromatic_monochromaticlightoutput);
  ...
  pow_sw1_green_led->set_pin(new GPIOPin(13, OUTPUT, true));
