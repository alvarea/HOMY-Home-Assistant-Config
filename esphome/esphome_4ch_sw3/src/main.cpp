...
void setup() {
  ...
  //   build_path: esphome_4ch_sw3
  //   platformio_options: {}
  //   esp8266_restore_from_flash: false
  //   includes: []
  //   libraries: []
  App.pre_setup("esphome_4ch_sw3", __DATE__ ", " __TIME__);
  // binary_sensor:
  // switch:
  // light:
  // logger:
  ...
  //     id: wifi_wifiap
  //     priority: 0.0
  //   use_address: 192.168.0.43
  wifi_wificomponent = new wifi::WiFiComponent();
  wifi_wificomponent->set_use_address("192.168.0.43");
  ...
  wifi_wifiap.set_manual_ip(wifi::ManualIP{
      .static_ip = IPAddress(192, 168, 0, 43),
  ...
  light_lightstate = new light::LightState("esphome 4ch sw3 blue led", monochromatic_monochromaticlightoutput);
  ...
  gpio_gpiobinarysensor->set_name("esphome 4ch sw3 button 1");
  App.register_binary_sensor(gpio_gpiobinarysensor_2);
  gpio_gpiobinarysensor_2->set_name("esphome 4ch sw3 button 2");
  App.register_binary_sensor(gpio_gpiobinarysensor_3);
  gpio_gpiobinarysensor_3->set_name("esphome 4ch sw3 button 3");
  App.register_binary_sensor(gpio_gpiobinarysensor_4);
  gpio_gpiobinarysensor_4->set_name("esphome 4ch sw3 button 4");
  App.register_binary_sensor(status_statusbinarysensor);
  status_statusbinarysensor->set_name("esphome 4ch sw3 Status");
  status_statusbinarysensor->set_device_class("connectivity");
  App.register_switch(gpio_gpioswitch);
  gpio_gpioswitch->set_name("esphome 4ch sw3 relay 1");
  App.register_switch(gpio_gpioswitch_2);
  gpio_gpioswitch_2->set_name("esphome 4ch sw3 relay 2");
  App.register_switch(gpio_gpioswitch_3);
  gpio_gpioswitch_3->set_name("esphome 4ch sw3 relay 3");
  App.register_switch(gpio_gpioswitch_4);
  gpio_gpioswitch_4->set_name("esphome 4ch sw3 relay 4");
